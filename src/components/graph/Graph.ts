...
export class Graph {
  vertices: Map<number, Node>; // Key = Node.id, Value = Node
  edges: Map<number | string, Way>; // Key = Way.id, Value = Way
  graph: Map<number, [number, number | string][]>; // Key = Node.id of the starting vertex, Value = [Neighbor/End vertex Node.id, Connecting Edge Way.id]

  constructor() {
    this.vertices = new Map<number, Node>();
    this.edges = new Map<number | string, Way>();
    this.graph = new Map<number, [number, number | string][]>();
  }
  ...
  addEdge(edge: Way, startVertex: Node, endVertex: Node) {
    if (!this.edges.has(edge.id)) {
      if (startVertex.id === 89116385 || endVertex.id === 89116385)
        console.log(edge, startVertex, endVertex);
      if (edge.id === "seg_82") console.log(edge, startVertex, endVertex);
